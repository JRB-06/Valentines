# Valentines

## About
Fun way to set up dates? 
This website features a way to set up a date through an interactive API
### Features:
#### Landing  
An invitation which features a YES or NO  
> It may or may not be interactive 👀

#### Where
Features an API call through [Mapbox](https://www.mapbox.com/) that displays a Map where clicking a point in the map returns its geolocation (coordinates) 
> this feature is just an initial development


## Development
__Developed with Vanilla HTML, CSS, and Javascript__  
<span>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/html5/html5-original.svg" height=100px width=100px/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/css3/css3-original.svg" height=100px width=100px/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/javascript/javascript-original.svg" height=100px width=100px/>
</span>

__API used__  
[Mapbox](https://www.mapbox.com/)
